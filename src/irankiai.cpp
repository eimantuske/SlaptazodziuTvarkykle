#include "irankiai.h"
#include <algorithm> // Reikia zodzioTaisymas funkcijai

bool SaugumoIrankiai::tikrinimasSlaptazodzio(const std::string& slaptazodis) {
    if (slaptazodis.length() < 8) return false;
    bool turiDidziaja = false, turiMaziaja = false, turiSkaiciu = false, turiSpecialu = false;
...
std::string SaugumoIrankiai::dabartinisLaikas() { 
    time_t dabar = time(0);
    struct tm tstruct;
    char buf[80];
...
void SaugumoIrankiai::zodzioTaisymas(std::string& svetaine) {
...
void SaugumoIrankiai::valytiEkrana(){
#ifdef _WIN32
    system("cls");
#else
    system("clear");
